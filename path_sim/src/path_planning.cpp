...
private:
    ...
    geometry_msgs::PoseStamped goal, robot_pose, next_pose, a_pose, b_pose, c_pose;
    bool is_arrived;
    int pos_type, hz;
...
PathPlaning ::PathPlaning()
{
    ...
    cmd_pub = nh.advertise<geometry_msgs::Twist>("/mobile_base/commands/velocity", 5, true);
    forward_pub = nh.advertise<nav_msgs::Odometry>("/path_sim/forward", 5, false);
    right_pub = nh.advertise<nav_msgs::Odometry>("/path_sim/right", 5, false);
    goal_pub = nh.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 5, false);
...
void PathPlaning::path_callback(const nav_msgs::PathConstPtr &path_msg)
{
    ...
    {
        path = *path_msg;
        int path_size = (30 < path_msg->poses.size()) ? 30 : path_msg->poses.size();
...
void PathPlaning::deal_str(string topic, geometry_msgs::PoseStamped *goal_msg)
{
    ...
    {
        index++;
        float x = stringToNum<float>(*iter);
...
void PathPlaning ::pose_callback(const nav_msgs::OdometryConstPtr &pose_msg)
{
    ...
    if (flag_path)
    {
        ...
        {
            cout << "flag_arriving_area  rotate" << endl;
            motor_control.linear.x = 0;
            if (flag_arriving_direction)
            {
        ...
        else
        {
            ...
            else
            {
                ...
                //calc rotate type
                Eigen::Quaterniond q(goal.pose.orientation.w, goal.pose.orientation.x, goal.pose.orientation.y, robot_pose.pose.orientation.z);
                Eigen::Vector3d v(1, 0, 0);
                Eigen::Vector3d f = q * v;

                next_pose.pose.position.x = goal.pose.position.x + f[0] * 2;
                next_pose.pose.position.y = goal.pose.position.y + f[1] * 2;
                next_pose.pose.position.z = goal.pose.position.z + f[2] * 2;
